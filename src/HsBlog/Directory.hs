...
convertDirectory env inputDir outputDir = do
  ...
  createOutputDirectoryOrExit outputDir (eReplace env)
  let
...
getDirFilesAndContent inputDir = do
  ...
  txtFilesAndContent <-
    applyIoAndReportFailures readFile txtFiles
  pure $ DirContents
  ...
  let
    previews =
      map
        ( \(file, doc) ->
          case doc of
            Markup.Heading 1 headTitle : article ->
              Html.h_ 3 (Html.link_ file (Html.txt_ headTitle))
...
createOutputDirectoryOrExit :: FilePath -> Bool -> IO ()
createOutputDirectoryOrExit outputDir replace =
  whenIO
    (not <$> createOutputDirectory outputDir replace)
    (hPutStrLn stderr "Cancelled." *> exitFailure)

-- | Creates the output directory.
--   Returns whether the directory was created or not.
createOutputDirectory :: FilePath -> Bool -> IO Bool
createOutputDirectory dir replace = do
  dirExists <- doesDirectoryExist dir
  create <-
    if dirExists
      then do
        override <- do
          if replace then pure True else confirm "Output directory exists. Override?"
...
copyFiles outputDir files = do
  ...
  void $ applyIoAndReportFailures copyFromTo files
...
writeFiles outputDir files = do
  ...
  void $ applyIoAndReportFailures writeFileContent files
...
-- | REFACTOR: Merge applyIoOnList & filterAndReportFailures
applyIoAndReportFailures :: (a -> IO b) -> [a] -> IO [(a, b)]
applyIoAndReportFailures action =
  foldMap $ \file -> do
    eitherResult <-
      catch
        (Right <$> action file)
        ( \(SomeException e) -> do
          Left <$> displayError (displayException e)
        )
